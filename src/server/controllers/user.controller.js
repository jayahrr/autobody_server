...
exports.create = ( req, res ) => {
  ...
  newCustomer.save().then(() => {
    ...
    .then(( token ) => {
      res.status( 201 ).header( 'x-auth', token ).json( newCustomer )
    })
    .catch(( e ) => 
...
exports.findMyVehicles = ( req, res ) => {
  VehicleInstance.find({ owner: req.user.id })
    .then(( myVehicles ) => {
      if ( !myVehicles ) {
    ...
    .catch(( e ) => 
      res.status( 400 ).send( apiErrorMsg( 'get', 'customer by Token', e )))
}
...
exports.findMe = ( req, res ) => {
  ...
  Customer.findByToken( req.token )
    .then(( me ) => {
      if ( !me ) {
        return res.status( 400 ).send({ message: 'No user found by token provided' })
      }

      return res.json( me )
    })
    .catch(( e ) =>
      res.status( 400 ).send( apiErrorMsg( 'get', 'customer', e )))
}